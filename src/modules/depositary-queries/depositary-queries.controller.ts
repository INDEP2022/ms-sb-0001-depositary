...
export class DepositaryQueriesController {
  ...
  @MessagePattern({ cmd: 'pagorefDepositariasRatificar' })
  async pagorefDepositariasRatificar({
    ...
    // PAGOREF_DEPOSITARIAS_RATIFICAR
    return await this.service.pagorefDepositariasRatificar({
      importe,
      aux_refe1,
    });
  }

  @MessagePattern({ cmd: 'pagorefDepositariasAuxRefe' })
  async pagorefDepositariasAuxRefe({ aux_refe1 }: { aux_refe1: string }) {
    return await this.service.pagorefDepositariasAuxRefe({
      aux_refe1,
    });
  }
}
